...
# ]
    ...
    def _initialize_actors(self, config):
        ...
        if adversary_actor:
            ...
            # --- 新增：设置 adversary_actor 的初始速度 ---
            # 获取车辆当前的向前向量
            adv_forward_vector = adversary_transform.get_forward_vector()
            # 使用 self._adversary_speed (单位通常为 m/s)
            adversary_actor.set_target_velocity(carla.Vector3D(
                x=adv_forward_vector.x * self._adversary_speed,
                y=adv_forward_vector.y * self._adversary_speed,
                z=adv_forward_vector.z * self._adversary_speed
            ))
    ...
    def _create_behavior(self):
        ...
        # 路径跟随行为
        # 注意：WaypointFollower 会在行为开始时接管速度控制
        keep_driving_1 = WaypointFollower(
            original_adversary, 
        ...
        # 将触发器和路径跟随加入序列
        # 在 trigger_1 成功之前，original_adversary 会维持在 _initialize_actors 中设置的物理状态（惯性行驶）
        original_sequence.add_children([trigger_1, keep_driving_1])
