...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True):
        ...
        self.config = config
        print(self.config.scenario_configs[0].type)
        ...
        # ========== 新增：生成 XML 中配置的 other_actors ==========
        self._temp_other_actors = []
        print("\n" + "=" * 60)
        print("【RouteScenario】开始生成 other_actors...")
        print("=" * 60)
        
        if hasattr(config, 'other_actors') and config.other_actors:
            ...
            for i, actor_conf in enumerate(config.other_actors):
                ...
                if actor:
                    ...
                    self._temp_other_actors.append(actor)
                else:
                    print(f"❌ 警告: 生成失败 {actor_conf.model}")
            print(f"\n总共成功生成 {len(self._temp_other_actors)} 个 Actor")
        else:
            print("⚠️ XML 中没有配置 other_actors")
        print("=" * 60 + "\n")
        # =========================================================
    ...
    def _initialize_actors(self, config):
        ...
        """
        # 添加临时生成的 other_actors（从 XML 配置）
        if hasattr(self, '_temp_other_actors'):
            for actor in self._temp_other_actors:
                self.other_actors.append(actor)
            print(f"\n✅ 已将 {len(self._temp_other_actors)} 个 XML 配置的 Actor 添加到场景中")
