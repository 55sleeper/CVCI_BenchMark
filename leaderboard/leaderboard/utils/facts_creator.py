...
def extract_lane_closure_facts(scenario, parent_scenario=None):
    ...
    facts['gate'] = compute_gate(facts['collision_count'])
    print(f"[DEBUG Facts] collision_count={facts['collision_count']}, gate={facts['gate']}", flush=True)

    facts['penalty'] = calculate_penalty_from_ttc(facts['min_ttc'])
    print(f"[DEBUG Facts] min_ttc={facts['min_ttc']}, penalty={facts['penalty']}", flush=True)
